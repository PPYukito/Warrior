...
void UHeroGameplayAbility_TargetLock::OnTargetLockTick(float DeltaTime)
{
	...
	if (bShouldOverrideRotation)
	{
		...
		LookAtRot -= FRotator(TargetLockOffsetPitch, TargetLockOffsetYaw, 0.f);
...
void UHeroGameplayAbility_TargetLock::GetAvailableActorToLock()
{
...
AActor* UHeroGameplayAbility_TargetLock::GetNearestTargetFromAvailableActors(const TArray<AActor*>& InAvialableActors)
{
	float ClosestToDistance = 0.f;
	return UGameplayStatics::FindNearestActor(GetHeroCharacterFromActorInfo()->GetActorLocation(), InAvialableActors, ClosestToDistance);
}
...
void UHeroGameplayAbility_TargetLock::GetAvailableActorsAroundTarget(TArray<AActor*>& OutActorsOnLeft, TArray<AActor*>& OutActorsOnRight)
{
	...
	for (AActor* AvailableActor : AvailableActorsToLock)
	{
		...
		if (CrossResult.Z > 0.f) // on the right or the current actor
		{
...
void UHeroGameplayAbility_TargetLock::SetTargetLockWidegetPosition()
{
